...
echo -e "${Blue}开始安装Redis、Mongodb等依赖${Font}"
    if [ "${release}" = "centos" ]; then
        yum -y install mongodb-org make git gcc crontabs
        wget http://download.redis.io/releases/redis-5.0.2.tar.gz
        tar zxvf redis-5.0.2.tar.gz && rm -rf tar xzf redis-5.0.2.tar.gz
        cd redis-5.0.2
        make
    if [ $? -ne 0 ]; then
        ...
        apt update -y
        apt-get install -y mongodb-org redis-server make git cron build-essential python-dev
    fi
    if [ $? -ne 0 ]; then
...
mkdir "/root/.aria2" /root/Download
...
mkdir /root/.caddy && touch /root/.caddy/caddy.log
